...
os.chdir(f"/net/zootopia/disk1/chaon/WORK/GxE/simuR1/power/pheno_fastgxe/")
...
dfE = pd.read_csv(f"../res_mask_fastGxE/envi.rm{rmE}.{rep}.txt", sep="\s+", header=None)
envi_lst = list(dfE.iloc[:, 1])
E = np.array(dfp.loc[:, envi_lst])
...
snp_indices = np.loadtxt(f"../pheno/{prefix}.rep_{rep}.power_snp_index.txt", dtype=np.int64).tolist()
snp_indices = [snp_indices]
print(len(snp_indices), "SNPs selected for analysis")
print(len(id_index_keep_lst), "samples selected for analysis")

subset_on_disk = snp_on_disk[id_index_keep_lst, snp_indices]

...
num_envi_lst = [1, 2, 5, 10, 20, 30, 40]
...
for num_envi in num_envi_lst:
    ...
    power_lst.append(["fastGxE", num_envi, np.sum(lst) / len(lst)])
...
for num_envi in num_envi_lst:
    ...
    for rep in range(1, 101):
        ...
        try:
            ...
            continue
    print(f"num_envi_{num_envi}: {len(lst)}")
    power_lst.append(["fastGxE-noNxE", num_envi, np.sum(lst) / len(lst)])
...
for num_envi in num_envi_lst:
    ...
    for rep in range(1, 101):
        ...
        try:
            ...
            continue
    print(f"num_envi_{num_envi}: {len(lst)}")
    power_lst.append(["StructLMM", num_envi, np.sum(lst) / len(lst)])
...
for num_envi in num_envi_lst:
    ...
    print(f"num_envi_{num_envi}: {len(lst)}")
    power_lst.append(["fastGWA-GE", num_envi, np.sum(lst) / len(lst)])


dfR = pd.DataFrame(power_lst, columns=["method", "num_envi", "power"])

dfR.to_csv(f"/net/zootopia/disk1/chaon/WORK/GxE/simuR1/power/res_power/nE.{prefix}.csv", index=False)

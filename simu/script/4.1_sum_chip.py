...
for h2_gxe in [1, 5, 10]:
    ...
    for rep in range(100):
        file = f"chip.h2_add_{h2_add}.h2_gxe_{h2_gxe}.h2_nxe_{h2_nxe}.sample_{sample_size}.rep_{rep+1}.var"
        try:
            arr = np.loadtxt(file)
        ...
        file = f"chip.h2_add_{h2_add}.h2_gxe_{h2_gxe}.h2_nxe_{h2_nxe}.sample_{sample_size}.rep_{rep+1}.noNxE.var"
        try:
            arr = np.loadtxt(file)
...
dfR.to_csv(f"../res_mom_sum/chip.Varying_h2_gxe.csv", index=False)
...
dfR = pd.DataFrame(res_dct)
dfR.to_csv(f"../res_mom_sum/chip.Varying_h2_nxe.csv", index=False)

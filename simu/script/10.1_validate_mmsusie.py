...
lead_snp = df_window.sort_values("p_gxe").iloc[0, 2]
G = MS.get_genotype(bedfile, [lead_snp], scale=True)
y = y - G @ np.linalg.lstsq(G, y, rcond=None)[0]

varcom = np.loadtxt(f"/net/zootopia/disk1/chaon/WORK/GxE/simuR1/null/res_fastgxe_testgxe/{prefix}.rep_{rep}.noNxE.var", dtype=float)[:, 0]
MS.cal_spVi(varcom)
...
MS.out_mmsusie(res, f"/net/zootopia/disk1/chaon/WORK/GxE/simuR1/null/validate_mmsusie/{rep}.{input_window_index}.mmsusie_out")

with open(f"/net/zootopia/disk1/chaon/WORK/GxE/simuR1/null/validate_mmsusie/{rep}.{input_window_index}.mmsusie_out.lead_snp", "w") as f:
    f.write(lead_snp + "\n")

pip, alpha, mu, sets = susie(EG, y)

np.savetxt(f"/net/zootopia/disk1/chaon/WORK/GxE/simuR1/null/validate_mmsusie/{rep}.{input_window_index}.susie.pip", pip)
np.savetxt(f"/net/zootopia/disk1/chaon/WORK/GxE/simuR1/null/validate_mmsusie/{rep}.{input_window_index}.susie.alpha", alpha)
np.savetxt(f"/net/zootopia/disk1/chaon/WORK/GxE/simuR1/null/validate_mmsusie/{rep}.{input_window_index}.susie.mu", mu)

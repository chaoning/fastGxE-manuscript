...
os.chdir("/net/zootopia/disk1/chaon/WORK/GxE/simuR1/null/pheno_rGE/")

# Read command line arguments
h2_envi = 0.05
power_snp_index = int(sys.argv[1])
power_snp_h2_add = float(sys.argv[2]) / 100
corR = float(sys.argv[3])  # correlation coefficient

h2_add = float(sys.argv[4]) / 100
h2_gxe = float(sys.argv[5]) / 100
h2_nxe = float(sys.argv[6]) / 100

if power_snp_h2_add > h2_add:
...
print("SNP additive effects for power analysis")
...
eff = np.random.normal(size=1)

eff_var = np.sum(np.var(snp_data * eff.reshape(1, -1), axis=0))
eff *= np.sqrt(power_snp_h2_add / eff_var)
add_power_sum = np.dot(snp_data, eff)
...
correlated_envi_arr = corR * snp_data + np.sqrt(1 - corR**2) * np.random.normal(size=snp_data.size)
correlated_envi_arr = (correlated_envi_arr - correlated_envi_arr.mean()) / correlated_envi_arr.std()
print(f"The correlation coefficient for G and the new environment is {np.corrcoef(snp_data, correlated_envi_arr)[0, 1]}")
dfP[known_envi_name] = correlated_envi_arr # Update the DataFrame with the new environment
...
eff_add *= np.sqrt((h2_add - power_snp_h2_add) / add_var)
...
y = add_power_sum + add_sum + GxE_sum + error + E_sum + nxe_sum

dfR = dfP.copy()
dfR["pheno"] = y

print("Saving results")
out_prefix = f"power_snp_h2_add_{sys.argv[2]}.rGE_{sys.argv[3]}.h2_add_{sys.argv[4]}.h2_gxe_{sys.argv[5]}.h2_nxe_{sys.argv[6]}.sample_{sample_size}.rep_{rep}"

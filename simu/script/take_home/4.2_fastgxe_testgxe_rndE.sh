...
#SBATCH --array=1-7000%500
#SBATCH --cpus-per-task=3
#SBATCH --time=10:00:00
#SBATCH --output=/net/zootopia/disk1/chaon/WORK/GxE/simuR1/script/take_home/Aout/fastgxe_testgxe_rndE/%a.txt
#SBATCH --error=/net/zootopia/disk1/chaon/WORK/GxE/simuR1/script/take_home/Aerr/fastgxe_testgxe_rndE/%a.txt
#SBATCH --mem-per-cpu=6000MB
#SBATCH --partition=mulan
#SBATCH --exclude=mulan-mc[01-02]

cd /net/zootopia/disk1/chaon/WORK/GxE/simuR1/power/res_fastgxe_testgxe_nEtested/
h2_add=30
h2_gxe=5
h2_nxe=15
...
k=0
for rep in {1..100}; do
	for rndE in 1 2 5 10 20 30 40;do
	
	arr=()
	while read -r col1; do
		arr+=("$col1")
	done < E.rep_$rep.rnd_$rndE.txt

		for num_envi_power in 2 30; do
			for power_snp_h2_gxe in 0.08; do
				for sample_size in 50000 100000 200000 300000 400000; do
					...
					if [ ${k} -eq ${SLURM_ARRAY_TASK_ID} ]; then
						...
						time fastgxe --test-gxe  \
							...
							--data ../pheno_take_home_fastgxe/$prefix.txt --trait trait$rep \
							--env-int ${arr[@]} \
							--threads 10 --num-random-snp 1000 --maxiter 100 \
							--out rndE$rndE.$prefix.rep_$rep \

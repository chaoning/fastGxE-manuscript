...
file = f"{rep}.{input_window_index}.mmsusie_out.alpha.txt"
alpha = np.loadtxt(file)
alpha = alpha.reshape(-1, 40)

file = f"{rep}.{input_window_index}.mmsusie_out.mu.txt"
mu = np.loadtxt(file)
mu = mu.reshape(-1, 40)
...
dfP = pd.read_csv(f"/net/zootopia/disk1/chaon/WORK/GxE/simuR1/null/pheno_fastgxe/{prefix}_part2.txt", sep=r"\s+")

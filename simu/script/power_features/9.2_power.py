...
file = f"/net/zootopia/disk1/chaon/WORK/GxE/simuR1/null/QQ_plot_baselineLD/{prefix}.csv"
df = pd.read_csv(file)
p_cutoff_fastGxE = df.iloc[1, 1] / num_snp
p_cutoff_StructLMM = df.iloc[1, 2] / num_snp
p_cutoff_fastGWA_GE = df.iloc[1, 3] / num_snp


power_snp_h2_gxe_lst = [0.02, 0.04, 0.06, 0.08, 0.1, 0.12, 0.14]
num_envi=30

dfR = pd.DataFrame({"h2_gxe": power_snp_h2_gxe_lst})

# fastGxE
os.chdir(f"/net/zootopia/disk1/chaon/WORK/GxE/simuR1/power/res_baselineLD_fastgxe/")
power_lst = []
...
# StructLMM
os.chdir("/net/zootopia/disk1/chaon/WORK/GxE/simuR1/power/res_baselineLD_structlmm/")
power_lst = []
for power_snp_h2_gxe in power_snp_h2_gxe_lst:
    lst = []
    for rep in range(1, 101):
...
# fastGWA_GE
os.chdir("/net/zootopia/disk1/chaon/WORK/GxE/simuR1/power/res_baselineLD_fastGWA/")
power_lst = []
for power_snp_h2_gxe in power_snp_h2_gxe_lst:
    lst = []
    for rep in range(1, 101):
...
dfR.to_csv(f"/net/zootopia/disk1/chaon/WORK/GxE/simuR1/power/res_power/baselineLD.{prefix}.csv", index=False)

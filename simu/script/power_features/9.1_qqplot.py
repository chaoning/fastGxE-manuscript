...
# fastGxE
os.chdir("/net/zootopia/disk1/chaon/WORK/GxE/simuR1/null/res_baselineLD_fastgxe_testgxe/")
...
os.chdir("/net/zootopia/disk1/chaon/WORK/GxE/simuR1/null/res_baselineLD_structlmm/")
...
#fastGWA-GE
os.chdir("/net/zootopia/disk1/chaon/WORK/GxE/simuR1/null/res_baselineLD_fastGWA/")
...
p_observed = np.zeros((len(p_expected_arr), 3))
for i in range(len(p_expected_arr)):
    p = p_expected_arr[i]
    p_observed[i, 0] = np.quantile(p_fastgxe_arr, p)
    p_observed[i, 1] = np.quantile(p_structlmm_arr, p)
    p_observed[i, 2] = np.quantile(p_fastGWA_arr, p)

dfR = pd.DataFrame({
    "Expected": p_expected_arr,
    "fastGxE": np.log10(p_observed[:, 0]) / np.log10(p_expected_arr),
    "StructLMM": np.log10(p_observed[:, 1]) / np.log10(p_expected_arr),
    "fastGWA-GE": np.log10(p_observed[:, 2]) / np.log10(p_expected_arr)
})


p_arr_lst = [p_fastgxe_arr, p_structlmm_arr, p_fastGWA_arr]
color_lst = ["#376795", "#06948E", "#e76254"]
label_lst = ["fastGxE", "structLMM", "fastGWA-GE"]
out_file = f"/net/zootopia/disk1/chaon/WORK/GxE/simuR1/null/QQ_plot_baselineLD/{prefix}"
...
dfR = pd.DataFrame({
    ...
    "StructLMM": p_observed[:, 1],
    "fastGWA-GE": p_observed[:, 2]
})

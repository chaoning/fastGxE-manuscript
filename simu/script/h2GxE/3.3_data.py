...
for rep in tqdm(range(1, 101)):
    ...
    dfE2.to_csv(f"../pheno_GENIE/indE.{prefix}.E.{rep}.txt", sep=" ", index=False)
    file = f"indE.{prefix}.rep_{rep}.txt"
    df = pd.read_csv(file, sep=r"\s+")
    df.columns = ["FID", "IID", "Pheno"]
    df = pd.merge(dfFam, df, on=["FID", "IID"], how="left")
    df.to_csv(f"../pheno_GENIE/indE.{prefix}.{rep}.txt", sep=" ", index=False)

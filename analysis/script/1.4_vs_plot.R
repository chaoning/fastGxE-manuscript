...
# Load data
data <- read.table("/net/zootopia/disk1/chaon/WORK/GxE/Analysis/R1/physical/results/variance.txt", header = TRUE)
...
fig <- ggplot(data_long, aes(x = factor(Model, levels = unique(Model)), y = h2_gxe, fill = Model)) +
  ...
  labs(title = "Physical traits", x = NULL, y = expression(h[GxE]^2)) +
  annotate("text", x = 1.5, y = max(data_long$h2_gxe) * 1.05, label = pval_text, size = 3)

# Save plot
out_file <- "/net/zootopia/disk1/chaon/WORK/GxE/Analysis/R1/physical/results/variance.GxEh2.PT.png"

...
file = f"{trait}.clumped.r20.01.clumped"
...
for i in range(envi_mat.shape[1]):
    ...
    var1 = np.var(G_sum) / num_snps
    var2 = np.var(GxE_sum) / num_snps
    res_lst.append([r, var1, var2])

df_res = pd.DataFrame(res_lst, columns=["r", "var1", "var2"])

os.chdir("/net/zootopia/disk1/chaon/WORK/GxE/Analysis/R1/plink/")
df_res.to_csv(f"{trait}.cov_singleE.r20.01.csv", index=False)

df_res_allE = pd.DataFrame([[np.corrcoef(G_sum, GxE_sum_allE)[0, 1], np.var(G_sum) / num_snps, np.var(GxE_sum_allE) / num_snps, num_snps]], 
                           columns=["r", "var1", "var2", "num_snps"])
df_res_allE.to_csv(f"{trait}.cov_allE.r20.01.csv", index=False)
